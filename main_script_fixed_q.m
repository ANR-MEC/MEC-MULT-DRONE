...
sum_L = [40, 50, 60, 70, 80] * 10^6;
main_task{1} = sum_L/I .* ones(I,1);

% time in seconds
T = linspace(2, 2.5, 6);
main_task{2} = T;

for m = 1:size(main_task,2)
    for t = 1:size(main_task{m},2)
    
        if m == 1
            params.L_i = main_task{m}(:,t);
            params.delta = 2 / N;
        else
            params.L_i = 4 * 10^6 * ones(I,1);
            params.delta = T(t) / N;
        ...
        fval4 = sum(sum(params.beta * params.delta * (params.theta1*veloc.^3 + params.theta2 ./ veloc)));
        ...
        while tol > max_tol && iteration <= max_iter
            ...
            % while tol_alg1 > max_tol && iteration_alg1 <= max_iter_alg1
                ...
                [B_op, prob4_output] = prob4_function_fixed_q(params, l_op, p_op, m_op, d_op_square, fval4, x0);     
                % fval4 = fval;
                prob4_output_list{iteration} = prob4_output;
        ...
        save(['fixed_q_prob2_output_', new_date, '.mat'], 'prob2_output_list')
        save(['fixed_q_prob4_output_', new_date, '.mat'], 'prob4_output_list')
        save(['fixed_q_params_', new_date, '.mat'], 'params')
