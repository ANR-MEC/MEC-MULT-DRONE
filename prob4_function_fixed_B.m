function [q_op, d_op_square, prob4_output, true_fval] = prob4_function_fixed_B(params, l_op, p_op, m_op, B_op, x0)    
    ...
    for n = 1:N-1
        x0.v_bar(:,n) = sqrt(sum((x0.q(:,:,n+1) - x0.q(:,:,n)).^2, 2) / params.delta^2);   
    end
    ...
    veloc = optimexpr(K,N-1);
    % for k = 1:K
    for n = 1:N-1
        veloc(:,n) = sqrt(sum((q(:,:,n+1) - q(:,:,n)).^2, 2)) / params.delta;   
    end
    % end

    prob4.Constraints.c13 = veloc <= params.V_max;
    ...
    % prob4.Constraints.c21 = l_op <= t .* Delta_t_mat;

    % constraint 24
    c4 = repmat(sqrt(1./m_op), [K,1,1]) .* params.Q_inv ./ log(2);
    R_bar_op = B_op .* (log2(1 + p_op .* params.beta_0 ./ ((params.H^2 + d_op_square) * params.sigma2)) - c4);
    taylor_t = R_bar_op - B_op .* (log2(exp(1)) .* p_op .* params.beta_0 .* (d_square - d_op_square)) ./ ...
        ((params.H^2+d_op_square) .* (params.sigma2 * (params.H^2+d_op_square) + p_op * params.beta_0));
    % prob4.Constraints.c24 = t <= taylor_t;
    
    prob4.Constraints.c21 = l_op <= taylor_t .* Delta_t_mat;
    ...
    % objective
    obj = sum(sum(params.beta * params.delta * (params.theta1*veloc.^3 + params.theta2 ./ v_bar)));
    prob4.Objective = obj; 
    ...
    sol_struct = struct(q = q_op, v_bar = v_bar_op);
    prob4_output = struct('sol', sol_struct, 'fval', fval, ...
        'exitflag', exitflag, 'output', output);

    true_veloc = evaluate(veloc, struct(q = q_op));
