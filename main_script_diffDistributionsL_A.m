...
figure; plot(params.w(:,1), params.w(:,2), '.');
a = [1:16]'; b = num2str(a); c = cellstr(b);
dx = 0.1; dy = 0.1; % displacement so the text does not overlay the data points
text(params.w(:,1)+dx, params.w(:,2)+dy, c);
...
idx_task(1,:) = [6,7,10,11];
task{1} = 2 * 10^6 * ones(I,1);
task{1}(idx_task(1,:)) = 10 * 10^6;

idx_task(2,:) = [1,4,13,16];
task{2} = 2 * 10^6 * ones(I,1);
task{2}(idx_task(2,:)) = 10 * 10^6;

% idx_task(3,:) = [11,12,15,16];
% task{3} = 2 * 10^6 * ones(I,1);
% task{3}(idx_task(3,:)) = 10 * 10^6;

% idx_task(4,:) = [0,0,0,0];
% task{4} = 4 * 10^6 * ones(I,1);
...
for t = 1:length(task)
	...
	if sum(idx_task(t,:), 'all') ~= 0
	    B_op(:,idx_task(t,:),:) = 7.5 * 10^6;
    end
    ...
    lin = true;
    obj = false;
    [f_op, g_op, l_op, p_op, m_op, ~, ~] = prob2_function(params, B_op, d_op_square, lin, obj, x0);
    ...
    while tol > max_tol && iteration <= max_iter
        ...
        lin = false;
        obj = true;
        [f_op, g_op, l_op, p_op, m_op, prob2_output, fval] = prob2_function(params, B_op, d_op_square, lin, obj, x0);
        fval2 = fval;
        ...
        while tol_alg1 > max_tol && iteration_alg1 <= max_iter_alg1
            ...
            [B_op, q_op, t_op, d_op_square, prob4_output, fval] = prob4_function(params, l_op, p_op, m_op, x0);     
            fval4 = fval;
    ...
    save(['prob2_output_', new_date, '.mat'], 'prob2_output_list')
    save(['prob4_output_', new_date, '.mat'], 'prob4_output_list')
    save(['params_', new_date, '.mat'], 'params')
