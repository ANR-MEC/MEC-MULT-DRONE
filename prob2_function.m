function [f_op, g_op, l_op, p_op, m_op, prob2_output, true_fval] = prob2_function(params, B_op, d_op_square, lin, obj, x0)    
    ...
    if ~lin
        ...
        m_op_aux = repmat(x0.m, [K,1,1]);
        % m_op = x0.m;
        c4 = sqrt(1./m_op_aux) .* params.Q_inv ./ log(2);

        prob2.Constraints.c_aux1 = l ./ (B_op(:,:,1:N-1) .* Delta_t_mat(:,:,1:N-1)) + c4(:,:,1:N-1) <= 10;
    ...
    if lin
        ...
        % p_op = x0.p;
        c4 = sqrt(1./m_op_aux) .* params.Q_inv ./ log(2);
        p_op(:,:,1:N-1) = 2*params.sigma2 ./ h(:,:,1:N-1) .* (2.^(l_op ./ (B_op(:,:,1:N-1) .* Delta_t_mat(:,:,1:N-1)) + c4(:,:,1:N-1)) - 1);
        p_op(:,:,N) = 2*params.sigma2 ./ h(:,:,N) .* (2.^c4(:,:,N) - 1);
    else
        m_op = reshape(sol(varindex(prob2).m), [1, I, N]);
        p_op = reshape(sol(varindex(prob2).p), [K, I, N]);
    end

    sol_struct = struct(f = f_op, g = g_op, l = l_op, p = p_op, m = m_op);
